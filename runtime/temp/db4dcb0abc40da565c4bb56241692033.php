<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:86:"F:\aliveDinner\upupw\www\aliveHouse\public/../application/manage\view\order\index.html";i:1497965800;s:94:"F:\aliveDinner\upupw\www\aliveHouse\public/../application/common\view\layouts\manage-main.html";i:1501605631;}*/ ?>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-inline">
                    <select name="status" lay-search >
                        <option value="0">全部</option>
                        <?php if(is_array($statuses) || $statuses instanceof \think\Collection || $statuses instanceof \think\Paginator): $i = 0; $__LIST__ = $statuses;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?>
                        <option value="<?php echo $key; ?>" <?php if($key == $param['status']): ?>selected<?php endif; ?>><?php echo $item; ?></option>
                        <?php endforeach; endif; else: echo "" ;endif; ?>
                    </select>
                </div>
                ...
                <div class="layui-input-inline">
                    <input type="text" value="<?php echo $param['key']; ?>" name="key" placeholder="地区|房源编号|楼盘名称|客户名" class="layui-input">
                </div>
...
<table class="layui-table" lay-even="" lay-skin="row">
    ...
    <tbody>
    <?php if(is_array($dataProvider) || $dataProvider instanceof \think\Collection || $dataProvider instanceof \think\Paginator): $key = 0; $__LIST__ = $dataProvider;if( count($__LIST__)==0 ) : echo "<tr class='empty'><td colspan='7' style='text-align: center;'> 没有数据 </td> </tr>" ;else: foreach($__LIST__ as $key=>$model): $mod = ($key % 2 );++$key;?>
    <tr data-key="<?php echo $model['id']; ?>">
        ...
        <td style="width: 250px;text-align: center;" class="action">
            <a href="javascript:void(0);">
                <button onclick="Site.showUrl('查看详情','/manage/order/view.html?id=<?php echo $model['id']; ?>','62.8%','82%','2',true,true,'LAY_TRAVEL_<?php echo $model['id']; ?>',['0.372','#000'])" class="layui-btn layui-btn-small" >
                <i class="fa fa-eye"></i>
                查看详情
                </button>
            </a>
            <a href="javascript:void(0);">
                <button onclick="Site.showDialog('设置确认','你确定更新此项状态吗?',function(){updateModel(<?php echo $model['id']; ?>);},true)" class="layui-btn layui-btn-small read-status <?php if($model['readed'] == 2): ?>had-readed<?php else: endif; ?>" >
                <i class="fa fa-cog"></i>
                <span>
    ...
    <?php endforeach; endif; else: echo "<tr class='empty'><td colspan='7' style='text-align: center;'> 没有数据 </td> </tr>" ;endif; ?>
    </tbody>
</table>
