...
class TravelController extends ManageController
{
    ...
    public function lineAction($id = 0,$time = 0)
    {
        if (request()->isAjax()){
            ...
            $query = \app\home\model\Gps::load()->where('SIM','1064849399578')->order('update_time','ASC');
            if (!empty($time)){
                $ret['time'] = $time;
        ...
        $endLng = 110;
        $endLat = 20.125;
        $start = \app\home\model\Gps::load()->where('SIM','1064849399538')->order('update_time','ASC')->limit(1)->select();
        $end = \app\home\model\Gps::load()->where('SIM','1064849399538')->order('update_time','DESC')->limit(1)->select();
