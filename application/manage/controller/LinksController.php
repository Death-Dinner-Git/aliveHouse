...
class LinksController extends ManageController
{
    ...
    public function indexAction($pageNumber = 1,$name = null, $type = null,$app = null)
    {
        ...
        $count = OnLinks::load()->where($where)->count();
    ...
    {
        $config = new OnLinks();
        $configList = OnLinks::getTypeList();
        $appList = OnLinks::getAppList();
        if ($this->getRequest()->isPost()){
            $data = (isset($_POST['OnLinks']) ? $_POST['OnLinks'] : []);
            $data['updated_at'] = date('Y-m-d H:i:s');
            $data['created_at'] = date('Y-m-d H:i:s');
            if ($data){
                $validate = OnLinks::getValidate();
    ...
    public function viewAction($id)
    {
        ...
        $model = OnLinks::load()->where(['id'=>$id])->find();
    ...
    public function updateAction($id)
    {
        ...
        $configList = OnLinks::getTypeList();
        $appList = OnLinks::getAppList();
        $model = OnLinks::load()->where(['id'=>$id])->where($where)->find();
        if (!$model){
            return '';
        }
        ...
        if ($this->getRequest()->isPost()){
            ...
            if ($data){
                ...
                if ($validate->check($data) && OnLinks::update($data,['id'=>$id])){
    ...
    public function deleteAction($id)
    {
        ...
        if ($this->getRequest()->isAjax()){
            $result = OnLinks::update(['is_delete'=>'0'],['id'=>$id]);
            if ($result){
