...
class DeleteController extends ManageController
{
    /**
     * @description 显示资源列表
    ...
    public function indexAction($pageNumber = 1,$name = null, $type = null,$app = null)
    {
        ...
        $typeList = DeleteLog::getTypeList();
        if (isset($typeList[0])){
            unset($typeList[0]);
        }
        if ($type && $type != ''){
        ...
        $count = DeleteLog::load()->where($where)->count();
    ...
    {
        $config = new DeleteLog();
        $configList = DeleteLog::getTypeList();
        $appList = DeleteLog::getAppList();
