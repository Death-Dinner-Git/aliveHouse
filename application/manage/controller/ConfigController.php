...
class ConfigController extends ManageController
{
    ...
    protected function initAction(){
        $modelPath = ROOT_PATH.'application/manage/model/';
        $validatePath = ROOT_PATH.'application/manage/validate/';
        if ($od = opendir($modelPath)){
            while (($file = readdir($od)) !== false)  //读取该目录内文件
            {
                ...
                if (!empty($className) && $className != 'Model'){
                    ...
                    $newPath = $validatePath.$class.'.php';
...
PRE;
                        @file_put_contents($newPath,$prefix);
                    }
                }
            }
            closedir($od);
